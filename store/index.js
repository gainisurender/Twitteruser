...
const actions = {
  async login({ commit }, data) {
    try {
      const res = await this.$axios.post(
        'http://localhost:5000/api/user/login?Email=randeep%40gmail.com&Password=password',
        data
      )
  ...
  async getAllComments({ commit, state }) {
    await this.$axios
      .get('http://localhost:5000/api/comment?id=' + state.post.id)
      .then((res) => {
        console.log(res.data)
        // this.$router.push('/');
  ...
  async createComment({ commit, state }, data) {
    const res = await this.$axios.post(
      'http://localhost:5000/api/comment?id=' + state.post.id,
      { commenttext: data }
    )
  ...
  async deleteComment({}, data) {
    const res = await this.$axios.delete(
      'http://localhost:5000/api/comment/id?id=' + data
    )
    console.log(res.data)
  },
  ...
  async updatePost({ state }, data) {
    const res = await this.$axios.put(
      'http://localhost:5000/api/post/' + state.post.id,
      { title: data }
    )
    console.log(res.data)
  },
  async updateUser({ state }, data) {
    const res = await this.$axios.put('http://localhost:5000/api/user/id', {
      full_name: data,
    })
  },
}
